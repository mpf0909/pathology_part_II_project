...
IFS= read -r DATA_DIR < <(sed -n "$((SLURM_ARRAY_TASK_ID + 1))p" final_directories.txt | tr -d '\r')
...
application="python /rds/user/mf774/hpc-work/part_II_project/hovernet/hovernet-conic/inference-results/final-test-inference/gen_lizard_inference_files_to_analyse.py --data_dir \"$DATA_DIR\" --out_dir \"$OUT_DIR\""
