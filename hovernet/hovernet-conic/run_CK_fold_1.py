...
if __name__ == "__main__":
    ...
    FOLD_IDX = 0
    WORKSPACE_DIR = '/rds/user/mf774/hpc-work/part_II_project/in-house/training-CK/'
    SAVE_ROOT = f'{WORKSPACE_DIR}/models/baseline/'

    splits = joblib.load('/rds/user/mf774/hpc-work/part_II_project/in-house/training-CK/splits.dat')

    def run_one_split_with_param_set(save_path, split_info, param_kwargs):
        mkdir(save_path)

        template_paramset = load_yaml('param/inhouse.yaml')
    ...
    save_path_ = f'{SAVE_ROOT}/last_fifteen_tars_fold_1{FOLD_IDX:02d}/'
    split_info = splits[FOLD_IDX]
    run_one_split_with_param_set(save_path_, split_info, {})

# to just run second phase
# 1: change param/inhouse.yaml pretrained backbone
# 2: in models/hovernet/opt.py change 'nr_epochs' and set pretrained as the same pretrained backbone in param/inhouse.yaml
# always ensure file save paths are safe and not going to erase work